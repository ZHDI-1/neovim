...
describe('fileio', function()
  ...
  it('opens large UTF-8 files with mmap and supports edits', function()
    ...
    local function expect_mmap_piece(min_revision, text_size)
      ...
      ok(stats.mmap_piece_write_raw_original_count >= 0)
  ...
  it('truncates mmap piece tails without rewriting original spans', function()
    local lines = {}
    for i = 1, 40000 do
      lines[i] = ('tail-truncate mmap line %05d plain ascii'):format(i)
    end
    ...
    eq(0, stats.mmap_piece_add_len)
    eq(#lines, fn.line('$'))
    eq(lines[#lines], fn.getline('$'))
    eq(table.concat(lines, '\n') .. '\n', read_file('Xtest_mmap_readfile'))

    fast_before = stats.mmap_piece_write_fast_count
    original_before = original_after
    command(('%d,$delete _'):format(#lines - 9))
    for _ = 1, 10 do
      table.remove(lines)
    end
    command('write')
  ...
  it('streams mmap piece writes from original and add spans', function()
    local lines = {}
    for i = 1, 40000 do
      lines[i] = ('write-span mmap line %05d plain ascii'):format(i)
    end
    ...
    lines[20000] = 'write-span mmap changed add buffer'
    command("call setline(20000, 'write-span mmap changed add buffer')")
    stats = api.nvim__buf_stats(0)
    local fast_before = stats.mmap_piece_write_fast_count
    local original_before = stats.mmap_piece_write_clone_range_count
      + stats.mmap_piece_write_copy_range_count
  ...
  it('keeps mmap piece tree editable while swapfile is enabled', function()
    ...
    command("call setline(6, 'still unjournaled dirty base edit')")
    stats = api.nvim__buf_stats(0)
    eq(true, stats.mmap_active)
    eq(true, stats.mmap_piece_tree)
    eq(false, stats.mmap_piece_journal_active)
    eq(true, stats.mmap_piece_journal_failed)
    eq(0, stats.mmap_piece_journal_record_count)
    eq('still unjournaled dirty base edit', fn.getline(6))
    ...
    command("call setline(7, 'journal reopened after clean write')")
    stats = api.nvim__buf_stats(0)
    eq(true, stats.mmap_piece_journal_active)
    eq(false, stats.mmap_piece_journal_failed)
    eq(true, stats.mmap_piece_journal_dirty)
    eq(1, stats.mmap_piece_journal_record_count)
    eq('journal reopened after clean write', fn.getline(7))
  ...
  it('recovers mmap literal substitute edits from a piece journal', function()
    local lines = {}
    for i = 1, 40000 do
      lines[i] = ('line%d ascii text for mmap substitute recovery'):format(i)
    end
    lines[1111] = 'journal substitute needle one'
    lines[2222] = 'journal substitute needle two needle'
    ...
    command([[%s/needle/restored/g]])
    lines[1111] = 'journal substitute restored one'
    lines[2222] = 'journal substitute restored two restored'
    ...
    eq(2, stats.mmap_piece_journal_record_count)
    eq(literal_count + 1, stats.mmap_substitute_literal_count)
    eq(lines[1111], fn.getline(1111))
    eq(lines[2222], fn.getline(2222))
    local journal_path = stats.mmap_piece_journal_path
    neq(nil, uv.fs_stat(journal_path))
    eq(true, uv.fs_copyfile(journal_path, 'Xtest_mmap_saved_journal'))
