...
static void ml_mmap_marked_clear(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL
{
  XFREE_CLEAR(buf->b_ml.ml_mmap_marked_ranges);
  buf->b_ml.ml_mmap_marked_count = 0;
  buf->b_ml.ml_mmap_marked_cap = 0;
  buf->b_ml.ml_mmap_marked_idx = 0;
}

static bool ml_mmap_marked_has(const buf_T *buf)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
  return buf->b_ml.ml_mmap_marked_idx < buf->b_ml.ml_mmap_marked_count;
}

static void ml_mmap_marked_drop_consumed(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL
{
  const size_t idx = buf->b_ml.ml_mmap_marked_idx;
  if (idx == 0) {
    return;
  }
  if (idx >= buf->b_ml.ml_mmap_marked_count) {
    buf->b_ml.ml_mmap_marked_count = 0;
    buf->b_ml.ml_mmap_marked_idx = 0;
    return;
  }

  const size_t remaining = buf->b_ml.ml_mmap_marked_count - idx;
  memmove(buf->b_ml.ml_mmap_marked_ranges,
          buf->b_ml.ml_mmap_marked_ranges + idx,
          remaining * sizeof *buf->b_ml.ml_mmap_marked_ranges);
  buf->b_ml.ml_mmap_marked_count = remaining;
  buf->b_ml.ml_mmap_marked_idx = 0;
}

static void ml_mmap_marked_reserve(buf_T *buf, size_t needed)
  FUNC_ATTR_NONNULL_ALL
{
  if (needed <= buf->b_ml.ml_mmap_marked_cap) {
    return;
  }

  size_t new_cap = buf->b_ml.ml_mmap_marked_cap == 0 ? 8 : buf->b_ml.ml_mmap_marked_cap;
  while (new_cap < needed) {
    new_cap *= 2;
  }
  buf->b_ml.ml_mmap_marked_ranges =
    xrealloc(buf->b_ml.ml_mmap_marked_ranges,
             new_cap * sizeof *buf->b_ml.ml_mmap_marked_ranges);
  buf->b_ml.ml_mmap_marked_cap = new_cap;
}

static void ml_mmap_marked_set(buf_T *buf, linenr_T lnum)
  FUNC_ATTR_NONNULL_ALL
{
  ml_mmap_marked_drop_consumed(buf);
  if (buf->b_ml.ml_mmap_marked_count > 0) {
    mmap_marked_range_T *last =
      &buf->b_ml.ml_mmap_marked_ranges[buf->b_ml.ml_mmap_marked_count - 1];
    if (lnum <= last->end) {
      return;
    }
    if (last->end < MAXLNUM && lnum == last->end + 1) {
      last->end = lnum;
      return;
    }
  }

  ml_mmap_marked_reserve(buf, buf->b_ml.ml_mmap_marked_count + 1);
  buf->b_ml.ml_mmap_marked_ranges[buf->b_ml.ml_mmap_marked_count++] =
    (mmap_marked_range_T){
      .start = lnum,
      .end = lnum,
    };
...
static void ml_mmap_marked_update_lowest(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL
{
  lowest_marked = ml_mmap_marked_has(buf)
                  ? buf->b_ml.ml_mmap_marked_ranges[buf->b_ml.ml_mmap_marked_idx].start
                  : 0;
}

static void ml_mmap_marked_insert(buf_T *buf, linenr_T lnum)
  FUNC_ATTR_NONNULL_ALL
{
  ...
  for (size_t i = 0; i < buf->b_ml.ml_mmap_marked_count; i++) {
    ...
    const linenr_T old_end = range->end;
    range->end = lnum;
    ml_mmap_marked_reserve(buf, buf->b_ml.ml_mmap_marked_count + 1);
    ranges = buf->b_ml.ml_mmap_marked_ranges;
    memmove(ranges + i + 2, ranges + i + 1,
            (buf->b_ml.ml_mmap_marked_count - i - 1) * sizeof *ranges);
    ranges[i + 1] = (mmap_marked_range_T){
      .start = lnum + 2,
      .end = old_end + 1,
    };
    buf->b_ml.ml_mmap_marked_count++;
    i++;
  }
  ml_mmap_marked_update_lowest(buf);
}

static void ml_mmap_marked_merge(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL
{
  if (buf->b_ml.ml_mmap_marked_count <= 1) {
    return;
  }
  ...
  size_t out = 0;
  for (size_t i = 0; i < buf->b_ml.ml_mmap_marked_count; i++) {
    if (out > 0 && ranges[i].start <= ranges[out - 1].end + 1) {
      ranges[out - 1].end = MAX(ranges[out - 1].end, ranges[i].end);
    } else {
      ranges[out++] = ranges[i];
    }
  }
...
static void ml_mmap_marked_delete(buf_T *buf, linenr_T lnum)
  FUNC_ATTR_NONNULL_ALL
{
  if (!ml_mmap_marked_has(buf)) {
    return;
  ...
  ml_mmap_marked_drop_consumed(buf);
  mmap_marked_range_T *ranges = buf->b_ml.ml_mmap_marked_ranges;
  for (size_t i = 0; i < buf->b_ml.ml_mmap_marked_count;) {
    mmap_marked_range_T *range = &ranges[i];
    if (range->end < lnum) {
      i++;
    } else if (range->start > lnum) {
      range->start--;
      range->end--;
      i++;
    } else {
...
static bool ml_mmap_piece_dispose_worker_active = false;
  ...
  FUNC_ATTR_NONNULL_ALL
{
  if (storage->tree != NULL) {
    ...
    if (!piece_tree_dispose_budget(storage->tree, budgetp)) {
      return false;
    }
  }
  return true;
...
static void ml_mmap_retired_piece_storage_dispose_finish_event(void **argv)
{
  (void)argv;
  ml_mmap_piece_dispose_worker_active = false;
...
static void ml_mmap_retired_piece_storage_dispose_worker(void *arg)
{
  ...
  if (main_loop.events != NULL && !main_loop.closing) {
    loop_schedule_deferred(&main_loop,
                           event_create(ml_mmap_retired_piece_storage_dispose_finish_event,
                                        NULL));
  }
}
...
static bool ml_mmap_retired_piece_storage_start_dispose(ml_mmap_retired_piece_storage_T *storage)
  FUNC_ATTR_NONNULL_ALL
{
  assert(!ml_mmap_piece_dispose_worker_active);
  ml_mmap_piece_dispose_worker_active = true;
  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  while (*storagep != NULL) {
    ...
    if (storage->tree != NULL && piece_tree_storage_ref_count(storage->tree) != 0) {
      has_more = true;
      storagep = &storage->next;
      continue;
    }
    if (ml_mmap_piece_dispose_worker_active) {
      break;
    }
  ...
  FUNC_ATTR_NONNULL_ALL
{
  if (buf->b_ml.ml_line_lnum != 0
      ...
      && buf->b_ml.ml_line_ptr != buf->b_ml.ml_mmap_line_buf) {
...
static void ml_mmap_free_line_buf(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL
{
  if (buf->b_ml.ml_line_ptr == buf->b_ml.ml_mmap_line_buf) {
    buf->b_ml.ml_line_ptr = NULL;
  }
  XFREE_CLEAR(buf->b_ml.ml_mmap_line_buf);
  buf->b_ml.ml_mmap_line_buf_cap = 0;
}

static char *ml_mmap_ensure_line_buf(buf_T *buf, size_t len)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  if (len > (size_t)MAXCOL - 1 || len == SIZE_MAX) {
    return NULL;
  }

  const size_t needed = len + 1;
  if (buf->b_ml.ml_mmap_line_buf_cap < needed) {
    size_t new_cap = buf->b_ml.ml_mmap_line_buf_cap == 0
                     ? 256
                     : buf->b_ml.ml_mmap_line_buf_cap;
    while (new_cap < needed) {
    ...
    buf->b_ml.ml_mmap_line_buf = xrealloc(buf->b_ml.ml_mmap_line_buf, new_cap);
    buf->b_ml.ml_mmap_line_buf_cap = new_cap;
  }
  return buf->b_ml.ml_mmap_line_buf;
}
...
static uint32_t ml_mmap_piece_journal_flags(const buf_T *buf)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
  return buf->b_ml.ml_mmap_noeol ? PIECE_JOURNAL_FLAG_NOEOL : 0;
}

static char *ml_mmap_piece_journal_name_from_swap(const char *swapname)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  const size_t len = strlen(swapname);
  char *journal = xmallocz(len + 3);
  memcpy(journal, swapname, len);
  memcpy(journal + len, ".pj", 3);
  return journal;
}
...
static char *ml_mmap_piece_journal_make_name(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  if (buf->b_fname == NULL || buf->b_ffname == NULL || p_dir == NULL || *p_dir == NUL) {
    return NULL;
  }
  ...
  char *journal = NULL;
  while (*dirp != NUL && journal == NULL) {
    copy_option_part(&dirp, dir_name, strlen(p_dir) + 1, ",");
    if (dir_name[0] == NUL) {
      continue;
    }
    if (dir_name[0] != '.' || dir_name[1] != NUL) {
      ...
      if (!os_isdir(dir_name)
          && os_mkdir_recurse(dir_name, 0755, &failed_dir, NULL) != 0) {
        xfree(failed_dir);
        continue;
      }
    }

    char *swapname = makeswapname(buf->b_fname, buf->b_ffname, buf, dir_name);
    if (swapname != NULL) {
      journal = ml_mmap_piece_journal_name_from_swap(swapname);
      xfree(swapname);
    }
  }
  xfree(dir_name);
...
static bool ml_mmap_piece_journal_write_all(int fd, const char *data, size_t len)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  size_t written = 0;
  while (written < len) {
    const ssize_t n = write_eintr(fd, (void *)(data + written), len - written);
    if (n <= 0) {
      return false;
    }
    written += (size_t)n;
...
static void ml_mmap_piece_journal_open(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL
{
  if (!buf->b_p_swf || p_uc == 0 || buf->b_ml.ml_mmap_piece_journal_fd >= 0
  ...
  char *journal_name = ml_mmap_piece_journal_make_name(buf);
  ...
  const char *path = buf->b_ffname;
  PieceJournalHeader header = {
    .path = path,
    .path_len = strlen(path),
    .original_dev = (uint64_t)file_info.stat.st_dev,
    .original_ino = (uint64_t)file_info.stat.st_ino,
    .original_size = os_fileinfo_size(&file_info),
    .original_mtime_sec = (uint64_t)file_info.stat.st_mtim.tv_sec,
    .original_mtime_nsec = (uint64_t)file_info.stat.st_mtim.tv_nsec,
    .text_size = (uint64_t)ml_mmap_text_size(buf),
    .line_count = (uint64_t)buf->b_ml.ml_line_count,
    .flags = ml_mmap_piece_journal_flags(buf),
  };
  const size_t size = piece_journal_header_encoded_size(&header);
  char *encoded = size == 0 ? NULL : xmalloc(size);
  size_t written = 0;
  const bool ok = encoded != NULL
                  && piece_journal_header_encode(&header, encoded, size, &written)
                  && written == size
                  && ml_mmap_piece_journal_write_all(fd, encoded, size);
  xfree(encoded);
  if (!ok) {
    close(fd);
    os_remove(journal_name);
    xfree(journal_name);
    buf->b_ml.ml_mmap_piece_journal_failed = true;
    return;
  }
...
static void ml_mmap_piece_journal_append(buf_T *buf, PieceJournalOp op, size_t offset,
                                         size_t delete_len, const char *insert_text,
                                         size_t insert_len)
  FUNC_ATTR_NONNULL_ARG(1)
{
  if (buf->b_ml.ml_mmap_piece_journal_fd < 0 || buf->b_ml.ml_mmap_piece_journal_failed) {
  ...
  PieceJournalRecord record = {
    .op = op,
    .revision = buf->b_ml.ml_piece_tree == NULL ? 0 : buf->b_ml.ml_piece_tree->revision,
    .offset = (uint64_t)offset,
    .delete_len = (uint64_t)delete_len,
    .insert_len = (uint64_t)insert_len,
    .text_size_after = (uint64_t)ml_mmap_text_size(buf),
    .line_count_after = (uint64_t)buf->b_ml.ml_line_count,
    .flags_after = ml_mmap_piece_journal_flags(buf),
  ...
  size_t written = 0;
  const bool ok = encoded != NULL
                  && piece_journal_record_encode(&record, encoded, size, &written)
                  && written == size
                  && ml_mmap_piece_journal_write_all(buf->b_ml.ml_mmap_piece_journal_fd,
                                                     encoded, size);
  xfree(encoded);
  if (!ok) {
    ml_mmap_piece_journal_fail(buf);
  ...
  FUNC_ATTR_NONNULL_ARG(1)
{
  ...
  if (delete_len == 0) {
    ml_mmap_piece_journal_append(buf, kPieceJournalOpInsert, offset, 0, insert_text, insert_len);
  } else {
    ml_mmap_piece_journal_append(buf, kPieceJournalOpReplace, offset, delete_len, insert_text,
                                 insert_len);
  }
}
  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  buf->b_ml.ml_mmap_piece_write_raw_original_count = 0;
  ...
  FUNC_ATTR_NONNULL_ARG(1, 3)
{
  ...
  ml_mmap_piece_journal_append_replace(buf, start, end - start, line, len);
  if (!copy && !noalloc) {
    xfree(line);
  }
  ml_mmap_clear_cache(buf);
  ...
  FUNC_ATTR_NONNULL_ARG(1)
{
  ...
  ml_mmap_marked_insert(buf, lnum);
  ml_mmap_update_noeol(buf);
  ml_mmap_piece_journal_append(buf, kPieceJournalOpInsert, offset, 0, insert, insert_len);
  xfree(insert);
  ml_mmap_clear_cache(buf);
  ml_mmap_maybe_schedule_piece_reclaim(buf);
  buf->b_ml.ml_flags &= ~ML_EMPTY;
  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  if (buf->b_ml.ml_line_count == 1) {
    ...
    const size_t old_total = ml_mmap_text_size(buf);
    if (!piece_tree_replace(buf->b_ml.ml_piece_tree, 0, old_total, "", 0)) {
      ml_mmap_clear_cache(buf);
      return FAIL;
    }
    ...
    ml_mmap_update_noeol(buf);
    if (old_total > 0) {
      ml_mmap_piece_journal_append(buf, kPieceJournalOpDelete, 0, old_total, NULL, 0);
    }
    ml_mmap_clear_cache(buf);
    ml_mmap_maybe_schedule_piece_reclaim(buf);
    buf->b_ml.ml_flags |= ML_EMPTY;
  ...
  buf->b_ml.ml_line_count--;
  ml_mmap_marked_delete(buf, lnum);
  ml_mmap_update_noeol(buf);
  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  ml_mmap_piece_journal_append_replace(buf, start, end - start, line, len);
  if (!noalloc) {
    if (line != buf->b_ml.ml_mmap_line_buf) {
      xfree(line);
    }
  }
  buf->b_ml.ml_flags &= ~(ML_LINE_DIRTY | ML_ALLOCATED);
...
bool ml_buf_mmap_compact_piece_tree(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  if (!ml_mmap_is_active(buf) || buf->b_ml.ml_piece_tree == NULL
      || buf->b_ml.ml_mmap_storage == NULL) {
  ...
  if (!piece_tree_clone_compact(new_tree, old_tree)) {
    xfree(new_tree);
    return false;
  }
  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  if (old_size == 0 || old_size >= piece_tree_byte_len(old_tree)
      || old_tree->original == NULL || old_tree->original[old_size - 1] != NL) {
    return FAIL;
  }
  ...
  size_t first_tail_lnum = 0;
  size_t tail_line_start = 0;
  if (!piece_tree_lnum_for_offset(old_tree, old_size, &first_tail_lnum, &tail_line_start)
      || tail_line_start != old_size || first_tail_lnum < 2) {
    return FAIL;
  }
  const size_t old_line_count = first_tail_lnum - 1;
  ...
  for (; filled < expected_index_count; filled++) {
    const size_t target_lnum = filled * (size_t)ML_MMAP_INDEX_STRIDE + 1;
    if (target_lnum > old_line_count) {
      break;
    }
    if (filled >= old_storage->line_index->count) {
      xfree(line_starts);
      return FAIL;
    }
    line_starts[filled] = old_storage->line_index->starts[filled];
  }

  PieceTreeSpanVec tail_vec = { 0 };
  if (!piece_tree_collect_span_vec_at_revision(old_tree, old_size, snapshot.byte_len - old_size,
                                               snapshot.revision, &tail_vec)) {
    xfree(line_starts);
    return FAIL;
  }

  size_t lnum = first_tail_lnum;
  if (filled < expected_index_count
      && filled * (size_t)ML_MMAP_INDEX_STRIDE + 1 == lnum) {
    line_starts[filled++] = old_size;
  }

  bool index_ok = true;
  for (size_t i = 0; i < tail_vec.count && index_ok; i++) {
    const PieceTreeSpan *span = &tail_vec.items[i];
    if (span->source != kPieceTreeSourceAdd) {
      index_ok = false;
      break;
    }

    const char *scan = span->data;
    const char *const end = span->data + span->len;
    while (scan < end) {
      ...
      const size_t next_offset = span->offset + (size_t)(nl - span->data) + 1;
      lnum++;
      if (next_offset < snapshot.byte_len
          && filled < expected_index_count
          && filled * (size_t)ML_MMAP_INDEX_STRIDE + 1 == lnum) {
        line_starts[filled++] = next_offset;
      }
      scan = nl + 1;
    }
  }
  piece_tree_span_vec_clear(&tail_vec);

  if (!index_ok || filled != expected_index_count) {
    xfree(line_starts);
    return FAIL;
  }
...
void ml_buf_mmap_piece_write_clone_range_record(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL
{
  if (ml_mmap_is_active(buf) && buf->b_ml.ml_piece_tree != NULL) {
    buf->b_ml.ml_mmap_piece_write_clone_range_count++;
  }
}
...
void ml_buf_mmap_piece_write_raw_original_record(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL
{
  if (ml_mmap_is_active(buf) && buf->b_ml.ml_piece_tree != NULL) {
    buf->b_ml.ml_mmap_piece_write_raw_original_count++;
  }
}
...
uint64_t ml_buf_mmap_piece_write_clone_range_count(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
  return ml_mmap_is_active(buf) ? buf->b_ml.ml_mmap_piece_write_clone_range_count : 0;
}
...
uint64_t ml_buf_mmap_piece_write_raw_original_count(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
  return ml_mmap_is_active(buf) ? buf->b_ml.ml_mmap_piece_write_raw_original_count : 0;
}
...
size_t ml_buf_mmap_line_buf_cap(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
  return ml_mmap_is_active(buf) ? buf->b_ml.ml_mmap_line_buf_cap : 0;
}

bool ml_buf_mmap_piece_journal_active(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
  return ml_mmap_is_active(buf) && buf->b_ml.ml_mmap_piece_journal_fd >= 0;
}

bool ml_buf_mmap_piece_journal_failed(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
...
uint64_t ml_buf_mmap_piece_journal_record_count(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
  return ml_mmap_is_active(buf) ? buf->b_ml.ml_mmap_piece_journal_record_count : 0;
}

uint64_t ml_buf_mmap_piece_journal_bytes(buf_T *buf)
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
...
bool ml_buf_mmap_piece_gc_worker_active(void)
  FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
  return ml_mmap_piece_dispose_worker_active;
}
...
int ml_open(buf_T *buf)
{
  ...
  buf->b_ml.ml_mmap_storage = NULL;
  buf->b_ml.ml_mmap_line_buf = NULL;
  buf->b_ml.ml_mmap_line_buf_cap = 0;
  buf->b_ml.ml_mmap_noeol = false;
  buf->b_ml.ml_mmap_source_is_buffer_file = false;
  buf->b_ml.ml_mmap_piece_journal_fname = NULL;
  buf->b_ml.ml_mmap_piece_journal_fd = -1;
  buf->b_ml.ml_mmap_piece_journal_record_count = 0;
  buf->b_ml.ml_mmap_piece_journal_bytes = 0;
  buf->b_ml.ml_mmap_piece_journal_failed = false;
...
errorret:
  ...
  if (ml_mmap_is_active(buf)) {
    ...
    if (buf->b_ml.ml_line_lnum != lnum) {
      ...
      if (!will_change) {
        char *line_buf = ml_mmap_ensure_line_buf(buf, len);
        if (line_buf == NULL) {
          goto errorret;
        }
        if (buf->b_ml.ml_piece_tree != NULL) {
          size_t copied = piece_tree_read(buf->b_ml.ml_piece_tree, start, line_buf, len);
          if (copied != len) {
            goto errorret;
          }
        } else {
          memmove(line_buf, buf->b_ml.ml_mmap_base + start, len);
        }
        line_buf[len] = NUL;
        buf->b_ml.ml_line_ptr = line_buf;
        buf->b_ml.ml_flags &= ~(ML_LINE_DIRTY | ML_ALLOCATED);
      } else if (buf->b_ml.ml_piece_tree != NULL) {
...
{
  if (ml_mmap_can_edit_tree(curbuf)) {
    if (lnum >= 1 && lnum <= curbuf->b_ml.ml_line_count) {
      if (lowest_marked == 0 || lowest_marked > lnum) {
        lowest_marked = lnum;
      }
      ml_mmap_marked_set(curbuf, lnum);
    }
    return;
  }
...
{
  if (ml_mmap_can_edit_tree(curbuf)) {
    if (ml_mmap_marked_has(curbuf)) {
      mmap_marked_range_T *range =
        &curbuf->b_ml.ml_mmap_marked_ranges[curbuf->b_ml.ml_mmap_marked_idx];
      const linenr_T lnum = range->start++;
      if (range->start > range->end) {
        curbuf->b_ml.ml_mmap_marked_idx++;
      }
      ml_mmap_marked_update_lowest(curbuf);
      return lnum;
    }
    lowest_marked = 0;
    return 0;
